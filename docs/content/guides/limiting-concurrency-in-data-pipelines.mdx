...
#### Throttling op/asset concurrency-limited runs

With a global concurrency limit set, you may notice that you will have active runs where no op is executing due to these concurrency constraints. This can unnecessarily waste resources or make it difficult to see which runs are actively making progress.

To throttle runs that will immediately be blocked from making progress on any of its steps due to global concurrency limits, you can configure your run queue to keep such runs in a `QUEUED` state until a slot frees up for one of its steps.
...
run_queue:
  should_block_op_concurrency_limited_runs: true
